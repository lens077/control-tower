...
- 网关路由 target **一律 `direct://<k8s Service>.ecommerce.svc:<port>`，不用 `discovery:///`**（dev 2026-09-03 e0c59ee，pre 2026-09-15）：业务服务已关闭 Consul 注册（各 deployment `CONSUL_ENABLED=false`），带 token 查 catalog 只剩 `consul` 自身，`discovery:///` 会因空实例列表让网关 `/readyz` 持续 503（2026-09-15 用 pre 模板滚公网网关时复现，已 `rollout undo`）。两种写法网关都支持（`proxy.go` 的 `discoveryPrefix`/`directPrefix`），重开注册时改回 `discovery:///` 即可；模板与 Config Center 里的 `gateway/<env>/routes.yaml` 键必须一起改，模板不会自动同步到线上。
- 集群镜像源**统一为 TCR**（`ccr.ccs.tencentyun.com/sumery/control-tower-*`，2026-09-15），不再从 GHCR 拉：GHCR 上的 config 镜像是 private（匿名拉取 401，见 2026-08-30 恢复实录），节点 containerd 还要维护 `NO_PROXY` 才能直连；TCR 与 ecommerce 同一注册表、`tcr-pull` Secret 现成。代价是每份 Deployment 必须带 `imagePullSecrets: [tcr-pull]`，`deploy/manifest_test.go` 守着。CI 发布 tag 仍双推，GHCR 只作追溯。
- 控制台 TS 生成物（`web/src/gen`）**由根级 `make api` 一并产出并进仓库**，不在前端构建时现生成：`web/buf.gen.yaml` 与根级 `buf.gen.ts.yaml` 曾各出一份、内容不一致（2026-09-15 实测 `system.proto` 的 `build_version` 字段在 `web/src/gen` 里缺了两周没人发现）。`web/buf.gen.yaml` 已删除，只认根级模板，`make check-gen`（CI `codegen` job）保证 Go/Connect/TS 三份与 proto 一致。
